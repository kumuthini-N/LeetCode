...
public class Arrays_FindPivotIndex {

    public static void main(String[] args) {
        Arrays_FindPivotIndex arr = new Arrays_FindPivotIndex();
        int[] nums = {1,7,3,6,5,6};
//        int[] nums = {1,2,3};
//        int[] nums = {2,1,-1};
//        int[] nums = {-1,-1,0,1,1,0};
        arr.findPivotIndex(nums);
    }
    ...
    private int findPivotIndex(int[] nums) {
        ...
        for(int i=0;i< nums.length;i++){
            ...
            leftSum = 0;
            rightSum = 0;
                while(leftCount > 0 && leftCount -1 < i){
                    leftSum += nums[leftCount-1];
                    leftCount--;
                }
                while(rightCount < nums.length-1){
                    rightSum += nums[rightCount+1];
                    rightCount++;
                } if(leftSum == rightSum){
                    return i;
